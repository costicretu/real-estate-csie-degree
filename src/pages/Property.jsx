...
export default function Property() {
    ...
    useEffect(() => {
        async function fetchListings() {
            try {
                ...
                const q = query(listingRef, where('property', '==', params.propertyName), orderBy('timestamp', 'desc'), limit(8))
    ...
    async function onFetchMoreListings() {
        try {
            const listingRef = collection(db, 'listings')
            const q = query(listingRef, where('property', '==', params.propertyName), orderBy('timestamp', 'desc'), startAfter(lastFetchListing), limit(4))
            const querySnap = await getDocs(q)
            const lastVisible = querySnap.docs[querySnap.docs.length - 1]
            setLastFetchListing(lastVisible)
            const listings = []
    ...
    return (
        <div className='max-w-6xl mx-auto px-3'>
            <h1 className='text-3xl text-center mt-6 font-bold'>
                {params.propertyName === 'apartment' ? 'Apartamente' : (params.propertyName === 'house' ? 'Case' : (params.propertyName === 'land' ? 'Terenuri' : ''))}
            </h1>
